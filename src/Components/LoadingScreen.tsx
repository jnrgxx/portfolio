...
export default function LoadingScreen({ onFinished }: LoadingScreenProps) {
  ...
  return (
    <div
      className={`fixed inset-0 z-[9999] flex flex-col items-center justify-center gap-6 transition-opacity duration-500 bg-bg ${
        fadeOut ? 'opacity-0 pointer-events-none' : 'opacity-100'
      }`}
    >
      {/* Terminal-style loader */}
      <div className="font-mono text-sm text-text-primary flex flex-col items-center gap-3">
        ...
        <p className="text-xs text-text-muted m-0 mt-1">loading assets...</p>
      </div>
    </div>
  )
}