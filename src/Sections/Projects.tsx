export default function Projects() {
    ...
    const projects: ProjectStructure[] = [
        ...
        {name: "2", desc: "RIASEC Team Matching Website", stack: ["PHP", "JavaScript", "TailwindCSS"], url: "https://github.com/jnrgxx/"},
    ...
    const certs: certStructure[] = [
        ...
        {name: "PCAP: Programming Essentials in Python", company: "Cisco Networking Academy & OpenEDG Python Institute", source: "Netacad", verified: true, url: "https://www.coursera.org/account/accomplishments/verify/MIOS4AP9U8C6"},
        {name: "Introduction to Software Engineering", company: "IBM", source: "Coursera", verified: true, url: "https://www.linkedin.com/in/jrogeet/details/certifications/1734626606926/single-media-viewer?type=DOCUMENT&profileId=ACoAAEJd4MEBcNifXTtCX7fSTLw2-bqBBsTXlC0&lipi=urn%3Ali%3Apage%3Ad_flagship3_profile_view_base_certifications_details%3BkUL923axQFCs%2FlHOFgF%2Bxg%3D%3D"},
    ]

    return (
        <section className="overflow-hidden relative flex flex-col min-h-hero-height border border-bd bg-cr p-section-pad">
             <div className="mb-5 tracking-wider border-[0.5px] border-acc py-[5px] px-[10px] flex gap-2 font-dm-mono text-xxs text-muted bg-abg">
                        ...
                        {projects.map((p, index) =>
                            <a key={p.url} href="#" className={`${p.url == "#" ? "opacity-40" : ""} flex flex-col p-3 bg-card border-[0.5px] border-bd`}>
                                ...
                                <div className="flex flex-col items-start">
                                    ...
                                    <a href={p.url} className="font-dm-mono text-xxs mt-1.5 text-sub">{p.url !== "#" ? p.url : ""}</a>
                ...
                <div className="flex w-full flex-col">
                    ...
                    <div className=" flex mb-2 justify-between">
                        <div className="flex gap-2 items-center">
                            ...
                            <h3 className="font-dm-mono text-xxs font-medium leading-3.5">CERTIFICATIONS</h3>
                        </div>
                    ...
                    {/* Certificates */}
                    {certs.map(c =>
                        <div key={c.url} className="flex justify-between bg-card px-2.5 py-2 border-[0.5px] border-bd mb-0">
                            <div className="flex flex-col items-start">
                                <h2 className="text-xs">{c.name}</h2>
                                <span className="font-dm-mono text-muted mt-0.5 text-xxs">{c.company} · {c.source}</span>
                            </div>

                            <div className="leading-none flex items-center font-dm-mono text-xxs text-acc bg-abg border-[0.5px] border-abr">verified ✓</div>
                        </div>
                       )}
                </div>
